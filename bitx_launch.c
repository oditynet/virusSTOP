...
static int set_attribute = 0;
static char attribute_value[2] = "1";  // По умолчанию "1"
...
void set_attribute_for_file(const char *filename, const char *value) {
    if (setxattr(filename, "user.bitX", value, 1, 0) == -1) {
        perror("setxattr failed");
        exit(EXIT_FAILURE);
    }
    printf("Set user.bitX=%s for %s\n", value, filename);
}
        ...
        switch (opt) {
            case 'v':
                ...
                if (strcmp(optarg, "0") == 0) {
                    strcpy(attribute_value, "0");
                } else if (strcmp(optarg, "1") == 0) {
                    strcpy(attribute_value, "1");
                } else {
                    fprintf(stderr, "Error: Invalid attribute value. Use 0 or 1.\n");
                    exit(EXIT_FAILURE);
                }
                break;
            default:
                fprintf(stderr, "Usage: %s [-v 0|1] [file] -- [program] [args...]\n", argv[0]);
                exit(EXIT_FAILURE);
        }
    }

    // После обработки опций, остаются аргументы программы
    if (optind < argc) {
        if (set_attribute) {
...
int main(int argc, char **argv) {
    ...
    // Если указан -v и файл, устанавливаем атрибут
    if (set_attribute && target_file) {
        set_attribute_for_file(target_file, attribute_value);
    }

    // Если после опций нет программы для запуска
    if (optind >= argc) {
        if (!set_attribute) {
            fprintf(stderr, "Error: No program specified\n");
            fprintf(stderr, "Usage: %s [-v 0|1] [file] -- [program] [args...]\n", argv[0]);
        }
        return set_attribute ? EXIT_SUCCESS : EXIT_FAILURE;
    }
