...
static int handle_creation(const char *pathname, int flags, mode_t mode, int (*orig_func)(const char *, int, ...)) {
    ...
    if (fd >= 0 && (flags & O_CREAT) && is_bitX_allowed()) {
        fsetxattr(fd, "user.bitX", attribute_value, 1, 0);
    }
    return fd;
}
...
// Основная функция для утилиты
int main(int argc, char *argv[]) {
    parse_arguments(argc, argv);
    
    if (set_attribute) {
        // Установка атрибута для указанных файлов
        for (int i = optind; i < argc; i++) {
            if (setxattr(argv[i], "user.bitX", attribute_value, 1, 0) == -1) {
                perror("setxattr failed");
                return EXIT_FAILURE;
            }
            printf("Set user.bitX=%s for %s\n", attribute_value, argv[i]);
        }
        return EXIT_SUCCESS;
    }
    
    // Если нет аргументов -v, продолжаем как LD_PRELOAD библиотека
    printf("Running as interceptor library. Use -v 0|1 to set attributes.\n");
    return EXIT_SUCCESS;
}
